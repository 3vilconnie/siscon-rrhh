    ...
    <Row className="g-4">
      ...
      <Col xs={12} lg={6}>
        <Card className="shadow-sm border-0 bg-white">
          <Card.Header className="bg-danger text-white fw-bold d-flex align-items-center justify-content-between py-3">
            <div className="d-flex align-items-center">
              <i className="bi bi-exclamation-triangle-fill me-2 fs-5"></i>
              <span>Funcionarios en Alerta de Continuidad ({alertas.length})</span>
            </div>
            <Badge pill bg="white" className="text-danger fw-bold small">
              Próxima Renovación
            </Badge>
          </Card.Header>

          <ListGroup variant="flush" className="overflow-auto" style={{ maxHeight: '650px' }}>
            {alertas.length === 0 ? (
              ...
              alertas.map((a) => (
                <ListGroup.Item
                  ...
                  className={`p-3 cursor-pointer transition-all border-bottom ${
                    seleccionado?.rut === a.rut
      ...
      <Col xs={12} lg={6} ref={detalleRef}>
        {seleccionado ? (
          <Card className="shadow-sm border-0 bg-white p-4 animate__animated animate__fadeIn">
            <div className="d-flex justify-content-between align-items-center border-bottom pb-2 mb-3">
              <div>
                ...
                <span className="text-muted font-monospace small">
                  Historial de Contratos (RUT: {seleccionado.rut})
                </span>
              </div>
                  ...
                  return (
                    <React.Fragment key={c.id}>
                      ...
                      <div className="position-relative mb-4 ps-4">
                        ...
                        <Card
                          className={`border-0 shadow-sm ${esVigente ? 'border-start border-3 border-success' : 'border-start border-3 border-secondary'}`}
                        >
                          <Card.Body className="p-3">
                            <div className="d-flex justify-content-between align-items-center mb-1">
                              <span className="fw-bold text-dark small">Contrato #{idx + 1}</span>
                              <Badge
                                bg={esVigente ? 'success' : 'secondary'}
                                style={{ fontSize: '0.65rem' }}
                              >
                                {esVigente ? 'Vigente' : 'Terminado'}
                              </Badge>
                            </div>
                            <Row className="g-1 text-muted" style={{ fontSize: '0.78rem' }}>
                              <Col xs={12}>
                                <i className="bi bi-calendar3 me-1"></i> {c.fecha_inicio} al{' '}
                                {c.fecha_termino || 'Indefinido'}
                              </Col>
                              <Col xs={12}>
                                <i className="bi bi-clock me-1"></i> Jornada: {c.jornada} Hrs.
                              </Col>
                              <Col xs={12}>
                                <i className="bi bi-currency-dollar me-1"></i> Sueldo Base: $
                                {parseFloat(c.sueldo_base?.toString() || '0').toLocaleString(
                                  'es-CL',
                                )}
                              </Col>
                            </Row>
          ...
          <div
            className="h-100 d-flex align-items-center justify-content-center p-5 border rounded bg-light border-dashed text-muted text-center small"
            style={{ minHeight: '350px' }}
          >
            <div>
              <i className="bi bi-clipboard2-pulse display-4 text-secondary mb-3 d-block"></i>
              Selecciona un funcionario de la nómina de advertencias para desplegar cronológicamente
              su línea temporal de contratos.
            </div>
          </div>
        )}
      </Col>
