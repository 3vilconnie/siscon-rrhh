// app/api/horas/generar/route.ts
// Rellena plantillas/horas-compensatorias.docx con Carbone y devuelve el detalle
// de horas compensatorias como PDF o DOCX.
// Requiere runtime Node y, solo para PDF, LibreOffice instalado en el servidor.

import { NextResponse } from 'next/server';
import path from 'node:path';
import fs from 'node:fs';
import type { DatosHoras } from '@/lib/horasCompensatorias';
...
const ARCHIVO_PLANTILLA = 'horas-compensatorias.docx';
...
export async function POST(request: Request) {
  ...
  if (!datos) {
    return NextResponse.json({ error: 'Faltan los datos del detalle.' }, { status: 400 });
  }
  ...
  try {
    ...
    const rutSlug = datos.trabajador?.rut_formateado?.replace(/[.-]/g, '') ?? 'documento';
    const nombreArchivo = `horas_compensatorias_${rutSlug}.${formato}`;
