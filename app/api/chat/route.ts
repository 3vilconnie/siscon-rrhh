...
export async function POST(req: Request) {
  try {
    ...
    const limpiarComando = (str: string) => {
      return str
        .replace(
          /(cuantos|cuántos|posee|tiene|registra|contratos|anexos|el funcionario|la funcionaria|buscar|busca a|encuentra a|quien es|quién es|datos de|ficha de|por favor)/g,
          '',
        )
        .replace(/\s+/g, ' ')
        .trim();
    ...
    if (quiereContratos || quiereDatos) {
      ...
      if (palabras.length === 0) {
        return NextResponse.json({
          respuesta:
            'Por favor, indícame el nombre o apellido del funcionario que deseas consultar.',
        });
      }

      // Consulta flexible con OR en Supabase
      let queryTrabajador = supabase
        .from('trabajadores')
        .select('rut, dv, nombres, primer_apellido, segundo_apellido');
      palabras.forEach((p) => {
        queryTrabajador = queryTrabajador.or(
          `nombres.ilike.%${p}%,primer_apellido.ilike.%${p}%,segundo_apellido.ilike.%${p}%`,
        );
      });
      ...
      if (!trabajadores || trabajadores.length === 0) {
        return NextResponse.json({
          respuesta: `No encontré ningún funcionario en los registros que coincida con la búsqueda de: **${nombreBuscar}**.`,
        });
      }
    ...
    return NextResponse.json({
      respuesta:
        'No logré comprender del todo la consulta. Prueba con expresiones como: \n\n• *"¿Cuántos contratos tiene Julia?"*\n• *"Ficha de datos de Alcon"*',
    });
  } catch (error: any) {
    return NextResponse.json(
      { respuesta: `Error interno de procesamiento: ${error.message}` },
      { status: 500 },
    );
  }
}
