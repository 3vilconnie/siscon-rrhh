...
import { construirDatosRecepcion, CAMPOS_RECEPCION } from '@/lib/recepcion';
...
export default function ModuloRecepcionDocumentos() {
  ...
  const generar = async (formato: 'pdf' | 'docx') => {
    ...
    try {
      const res = await fetch('/api/recepcion/generar', {
        method: 'POST',
      ...
      a.href = url;
      a.download = `Recepcion_Documentos.${formato}`;
      ...
      <Row className="g-4 justify-content-center">
        <Col lg={9}>
          <Card className="shadow-sm border-0">
            <Card.Body className="p-4 p-md-5" style={{ minHeight: '297mm' }}>
              <Row className="mb-5">
                <header>
              ...
              <Row className="mb-5 pb-4 border-bottom">
                <Col xs={12} md={6} className="mb-4 mb-md-0">
                  ...
                  <div className="d-flex flex-column gap-2">
                    {['Finiquito', 'Contrato', 'Anexo contrato', 'Notificación', 'Otro'].map(
                      (tipo) => (
                        <div key={tipo}>
                          <Form.Check
                            type="checkbox"
                    ...
                    <Table bordered hover className="align-middle text-nowrap mb-0">
                      <thead style={{ backgroundColor: '#8b0000', color: 'white' }}>
                        <tr>
                          ...
                          <th className="py-2 fw-normal">ACCIÓN</th>
                        </tr>
                      </thead>
                      <tbody>
                        {detalles.length == 0 ? (
                            ...
                            <tr key={fila.id}>
                              ...
                              <td className="p-1">
                                <Form.Control
                                  ...
                                  value={fila.cabecera}
                                  onChange={(e) =>
                                    handleInputChange(index, 'cabecera', e.target.value)
                                  }
                                  className="border-0 bg-transparent shadow-none"
                                  placeholder="PZD1"
                                />
              ...
              {/* Acciones */}
              <div className="d-flex justify-content-end align-items-end gap-2 mt-5">
                <Button
                  type="button"
        ...
        {/* PANEL LATERAL: marcadores de la plantilla Word */}
        <Col lg={2}>
          <Card className="shadow-sm border-0" style={{ position: 'sticky', top: '1rem' }}>
            ...
            <Card.Body className="p-3" style={{ maxHeight: '75vh', overflowY: 'auto' }}>
              <p className="text-muted" style={{ fontSize: '0.72rem' }}>
                Escribe estos marcadores en <code>plantillas/recepcion-documentos.docx</code>.
                Carbone los reemplaza al generar. La fila de la tabla con{' '}
                <code>{'{d.detalles[i]...}'}</code> se repite por cada documento.
              </p>
              {CAMPOS_RECEPCION.map((c) => (
